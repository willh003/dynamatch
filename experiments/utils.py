...
def init_wandb(config, job_type, tags=[], run_id_override=None, name_override=None):
    ...
    if config.resume and os.path.exists(run_id_path):
        ...
        # Use provided run_id override (e.g., timestamp) or generate a new one
        run_id = run_id_override or wandb.util.generate_id()
    ...
    wandb.init(
        ...
        job_type=job_type,
        group=config.algo,
        name=name_override or "_".join([config.exp_id, str(config.seed)]),
